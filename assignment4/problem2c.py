...
data = sc.textFile(args.data_path)
...
histogram = (
    values.map(lambda value: (get_bin_index(value, args.min_value, args.max_value, args.num_bins), 1))
          .reduceByKey(lambda a, b: a + b)
          .sortByKey()
)

# Collect the bin counts for the histogram and printing it
histogram_counts = histogram.collect()
for bin_index, count in histogram_counts:
    bin_range = get_bin_range(bin_index, args.min_value, args.max_value, args.num_bins)
