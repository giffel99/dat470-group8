...
samples_per_length = 10000
test_strings = [''.join(random.choices(alphanumeric_chars, k=length)) for length in range(1, 101) for _ in
...
plt.title(f'Hash value distribution, {samples_per_length * 100} samples')
plt.savefig('1b_histogram.png')
plt.show()
